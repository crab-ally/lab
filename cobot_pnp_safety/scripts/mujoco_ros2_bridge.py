...
class MjcfBridgeNode(Node):
    def __init__(self, model, data):
        ...
        # ------------------------------------------------------------------
        # 3. FollowJointTrajectory Action Server (MoveIt2 panda_arm_controller)
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # 4. GripperCommand Action Server (MoveIt2 panda_hand_controller)
        # ------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------
    # Arm Trajectory Action Callbacks
    # ----------------------------------------------------------------------
    ...
    def execute_arm_trajectory(self, goal_handle):
        ...
        for name in joint_names:
            if name in ARM_JOINT_NAMES:
                joint_indices.append(ARM_JOINT_NAMES.index(name))
            else:
        ...
        while rclpy.ok():
            ...
            # 현재 시간에 적합한 trajectory point 보간
                ...
                with self.lock:
                    for i, j_idx in enumerate(joint_indices):
                        if 0 <= j_idx < 7 and i < len(p0.positions) and i < len(p1.positions):
                            pos_interp = p0.positions[i] + alpha * (p1.positions[i] - p0.positions[i])
                            self.data.ctrl[j_idx] = float(pos_interp)
    ...
    def execute_gripper_command(self, goal_handle):
        pos = goal_handle.request.command.position  # 0.04 (open) ~ 0.0 (close)
        # MuJoCo Panda actuator8 ctrlrange: 0 ~ 255
        # pos=0.04 -> ctrl=255, pos=0.0 -> ctrl=0
...
def main(args=None):
    ...
    # ROS Spin을 별도 스레드에서 실행하여 Action Server 콜백 처리
