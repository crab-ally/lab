...
def randomize_scene():
    ...
    fj_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, "forklift_freejoint")
    if fj_id >= 0:
        qpos_adr = model.jnt_qposadr[fj_id]
        if random.random() < 0.2:
            # 20% hide
            data.qpos[qpos_adr:qpos_adr+3] = [100, 100, -10]
        else:
            for _ in range(100):
                fx = np.random.uniform(-4.0, 4.0)
                fy = np.random.uniform(-4.0, 4.0)
                if is_valid_position(fx, fy, 0.8):
                    break
            data.qpos[qpos_adr:qpos_adr+3] = [fx, fy, 0.0]
            fyaw = np.random.uniform(0, 2*np.pi)
            data.qpos[qpos_adr+3:qpos_adr+7] = [np.cos(fyaw/2), 0, 0, np.sin(fyaw/2)]
