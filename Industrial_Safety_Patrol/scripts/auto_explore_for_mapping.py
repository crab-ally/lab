...
class AutoExploreNode(Node):
    def __init__(self):
        ...
        raw_world_waypoints = [
            ...
            (-4.0, -3.0, 1.0),    # 서쪽벽 하단 + 남서 코너 스캔
            (-2.9, -3.0, 2.5),    # [점검] 쓰러진 작업자 남쪽 정밀관측
            (-1.0, -2.9, 1.0),    # 교차점A: 기둥1 근접통과 + 루프클로저 지점

            # 2. PPE 미착용 작업자 & 전기실(출입금지) 외벽 스캔
            (1.2, -3.6, 2.5),     # [점검] PPE 미착용자 남서쪽 관측
            (2.8, -3.6, 0.5),     # 전기실 서쪽벽 진입 통로
            (2.8, -4.3, 2.0),     # [점검] 전기실 서쪽벽+남서 심층스캔
            (2.8, -1.3, 1.0),     # 전기실 북쪽벽 통로 진입 (기둥3 서측 우회)
            (3.6, -1.3, 2.0),     # [점검] 동쪽선반1 정면 스캔

            # 3. 동쪽 선반열, 정상 작업자(PPE), 화재구역
            (2.7, 1.6, 2.5),      # [점검] 정상 작업자 PPE 근접관측
            (3.5, 2.4, 2.0),      # [점검] 동쪽선반2 정면 스캔
            (2.6, 2.6, 0.0),      # 화재구역 우회 경유점
            (2.4, 3.2, 3.0),      # [점검] 화재구역 남서 정밀스캔

            # 4. 북쪽 통로, 컨베이어벨트, 북서 코너
            (0.0, 3.8, 1.0),
            (-2.0, 3.8, 1.0),
            (-3.8, 3.8, 1.0),

            # 5. 서쪽 선반, 지게차 상단면
            (-3.65, 2.0, 2.0),    # [점검] 서쪽선반 정면 스캔
            (-2.5, 1.1, 2.0),     # [점검] 지게차 상단면 스캔

            # 6. 중앙 장애물, 지게차 하단면 (기둥2 우회)
            (-0.8, 1.4, 1.0),
            (-0.8, -0.5, 0.0),    # 중앙장애물 서측 경유
            (0.0, -0.9, 2.0),     # [점검] 중앙 방치장애물 남쪽 정밀스캔
            (-2.5, -1.0, 1.0),    # [점검] 지게차 하단면 스캔

            # 7. 남측 우회, 루프클로저, 충전소 복귀
            (-2.9, -1.5, 0.0),
            (-2.9, -2.6, 0.0),    # 쓰러진 작업자 서측 우회
            (-1.0, -2.9, 1.0),    # [SLAM 루프클로저] 교차점A 재방문
            (-4.0, -3.0, 1.0),
            (-4.0, -4.0, 5.0)     # [HOME] 충전소 안착
        ]
