...
def randomize_scene():
    ...
    for i, bid in enumerate(fallen_ids):
        ...
        body_gid = mujoco.mj_name2id(
            model, mujoco.mjtObj.mjOBJ_GEOM, f"fallen_worker_{i}_body"
        )
        if body_gid >= 0:
            model.geom_rgba[body_gid][:3] = random_color([0.2, 0.24, 0.28])
    ...
    for i, bid in enumerate(standing_ids):
        ...
        # 색상 랜덤화
        body_gid = mujoco.mj_name2id(
            model, mujoco.mjtObj.mjOBJ_GEOM, f"standing_worker_{i}_body"
        )
        if body_gid >= 0:
            model.geom_rgba[body_gid][:3] = random_color([0.2, 0.24, 0.28])
...
for i in range(NUM_DATA):
    ...
    for w in workers:
        ...
        idx    = w["id"]
        prefix = f"fallen_worker_{idx}"

        body_ratio = visible_ratio([f"{prefix}_body"], prefix)
        if body_ratio < 0.3:
            continue

        geom_names = [f"{prefix}_body", f"{prefix}_head"]
    ...
    for w in workers:
        ...
        idx    = w["id"]
        prefix = f"standing_worker_{idx}"

        head_ratio = visible_ratio([f"{prefix}_head"], prefix)
        if head_ratio < 0.5:
            continue
