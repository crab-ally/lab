...
class FireFusionNode(Node):
    def __init__(self):
        ...
        self.sub_candidates = self.create_subscription(
            String, '/fire_candidates', self._candidates_callback, 10
        )
        ...
        self.pub_fire_tracks = self.create_publisher(
            String, '/fire_tracks_3d', 10
        )
        self.pub_markers = self.create_publisher(
            MarkerArray, '/fire_fusion/debug_markers', 10
        )

        self.get_logger().info(
            'Fire Fusion Node (Depth + LiDAR + TF + Plane Validation) started.'
        )
    ...
    def _camera_info_callback(self, msg: CameraInfo):
        ...
        self.get_logger().info(
            f'CameraInfo loaded: fx={self.fx:.1f}, fy={self.fy:.1f}, '
            f'cx={self.cx:.1f}, cy={self.cy:.1f}, frame={self.camera_frame_id}'
        )
    ...
    def _candidates_callback(self, msg: String):
        ...
        try:
            ...
            self.get_logger().warn(
                f'TF lookup failed: {self.camera_frame_id} -> '
    ...
    def _calculate_3d_from_depth(
        ...
        """BBox 중앙 50% ROI의 Depth를 3D 좌표로 변환."""
        ...
        if self.latest_depth_encoding == '16UC1':
            ...
            valid_depths = depth_roi[
                np.isfinite(depth_roi) & (depth_roi > 0.1)
            ]
    ...
    def _get_lidar_distance(
        ...
        """Camera 좌표의 수평각에 대응하는 LiDAR 거리 반환."""
        ...
        if not (
            scan.range_min <= scan_dist <= scan.range_max
        ):
            return None
    ...
    def _check_plane_geometry(
        ...
        """
        ROI Depth를 3D Point로 변환하고 SVD로 평면성을 검사한다.
